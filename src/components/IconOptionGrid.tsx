        ...
        {field.options.map((option) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 rounded-[6px] border px-2.5 py-2 text-left transition-colors duration-[250ms] ease-ft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ft-purple/50 ${
                active
                  ? 'border-ft-purple bg-ft-purple/5 text-ft-purple'
                  : 'border-ft-ink/15 bg-white/40 text-ft-ink hover:border-ft-purple/50'
              }`}
            >
              {option.icon && <Icon name={option.icon} className="h-4.5 w-4.5 shrink-0" />}
              <span className="min-w-0 truncate font-mono text-[10px] uppercase leading-tight tracking-[0.04em]">
                {option.name}
              </span>
            </button>
          )
